...
class SimplePostLogger extends SimpleLogger
{
    ...
    function on_save_post($post_ID)
    {
        ...
        $args = array(
            ...
            'old_status' => get_post_status($old_post),
	...
	function add_post_data_diff_to_context(
		...
		foreach ($new_meta as $meta_key => $meta_value) {
			if (!isset($old_meta[$meta_key])) {
                ...
                if (stripos($meta_key, '_acf_') !== 0) {
                    $meta_changes['added'][$meta_key] = [
                        'old' => is_array($old_meta[$meta_key]) ? current($old_meta[$meta_key]) : json_encode($old_meta[$meta_key]),
                        'new' => is_array($new_meta[$meta_key]) ? current($new_meta[$meta_key]) : json_encode($new_meta[$meta_key]),
                    ];
                }
			}
		}
		...
		if ('post_updated' == $message_key) {
			...
			if (
				...
				if (isset($context['post_meta_added'])) {
                    $context['post_meta_added'] = json_decode($context['post_meta_added'], true);

					$meta_changed_out .=
						"<span class='SimpleHistoryLogitem__inlineDivided'>" .
						(int) count($context['post_meta_added']) .
						' added</span> ';
				}

				if (isset($context['post_meta_removed'])) {
                    $context['post_meta_removed'] = json_decode($context['post_meta_removed'], true);

					$meta_changed_out .=
						"<span class='SimpleHistoryLogitem__inlineDivided'>" .
						(int) count($context['post_meta_removed']) .
						' removed</span> ';
				}

				if (isset($context['post_meta_changed'])) {
                    $context['post_meta_changed'] = json_decode($context['post_meta_changed'], true);

					$meta_changed_out .=
						"<span class='SimpleHistoryLogitem__inlineDivided'>" .
						(int) count($context['post_meta_changed']) .
						' changed</span> ';
				}
                ...
                if (isset($context['post_meta_added'])) {
                    foreach ($context['post_meta_added'] as $meta_key => $meta_value) {
                        $diff_table_output .= sprintf(
                            '<tr><td>%1$s</td><td>%2$s</td></tr>',
                            $meta_key,
                            simple_history_text_diff(
                ...
                if (isset($context['post_meta_changed'])) {
                    foreach ($context['post_meta_changed'] as $meta_key => $meta_value) {
                        $diff_table_output .= sprintf(
                            '<tr><td title="%1$s">%1$s</td><td>%2$s</td></tr>',
                            $meta_key,
                            simple_history_text_diff(
                                $meta_value['old'],
                                $meta_value['new']
